...
/* ── Seed data ── */
const INITIAL_TXNS = [
  { id: 1, date: 'Dec 27', category: 'Food',     amount: 50.0 },
  { id: 2, date: 'Dec 27', category: 'Housing',  amount: 320.0 },
  { id: 3, date: 'Dec 27', category: 'Wellness', amount: 45.0 },
  { id: 4, date: 'Dec 24', category: 'Food',     amount: 18.5 },
  { id: 5, date: 'Dec 22', category: 'Travel',   amount: 210.0 },
  { id: 6, date: 'Nov 27', category: 'Housing',  amount: 1000.0 },
  { id: 7, date: 'Nov 23', category: 'Food',     amount: 125.0 },
  { id: 8, date: 'Nov 23', category: 'Misc',     amount: 30.0 },
];
...
export default function ExpensesPage() {
  ...
  const handleSubmit = () => {
    ...
    setAmountError('');
    const label = formatDate(date) || new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

    if (editingId !== null) {
      setTxns((prev) => prev.map((t) => (t.id === editingId ? { ...t, date: label, category, amount: value } : t)));
      setEditingId(null);
    } else {
      const nextId = txns.length ? Math.max(...txns.map((t) => t.id)) + 1 : 1;
      setTxns((prev) => [{ id: nextId, date: label, category, amount: value }, ...prev]);
    }
    setAmount('');
    setDate('');
    setCategory('Food');
